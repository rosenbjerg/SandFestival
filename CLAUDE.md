...
Viewport pinning (scrolled-up sessions not getting yanked to the bottom by streaming output) is **SwiftTerm's job as of 1.14.0** — don't reintroduce a `dataReceived` override for it. `SessionTerminalView` does override `dataReceived` again, but only to *observe*: it compares the public `scrollPosition` before and after the feed and, when the viewport is scrolled up and the position dropped, reports "output below viewport" for the detail pane's **New output** pill. It never sets scroll state. Two details are load-bearing: the before/after compare (not "bytes arrived") is what keeps Claude Code's in-place spinner redraws from raising the pill, and the flag is forwarded only on change — `scrolled(source:position:)` fires per scrolled line while pinned at the bottom, and the SwiftUI listener re-renders on every call it gets. `terminal.userScrolling` is internal to SwiftTerm, so `canScroll && scrollPosition < 1` is the "scrolled up" test. `AppleTerminalView.updateUserScrollingState` sets `terminal.userScrolling` from every `scrollTo`/`scroll(toPosition:)` — which is what `Terminal.scroll` has always gated on — and the wheel path reaches it via `scrollUp`/`scrollDown`. That also makes the scrollback-full case exact: the trim branch decrements `yDisp` per trimmed line to hold the text still, where our old override could only clamp and so drifted toward the bottom. Typing resets it, because `TerminalView.send(data:)` calls `ensureCaretIsVisible`. Through 1.13 the view layer set only its own same-named property (scrollbar-knob drags, never read), which is why the workaround existed at all; we carried it until the 1.18.0 bump only because the pin before that was an untagged May 2026 `main` revision predating the fix. If pinning regresses after a bump, check that wiring before re-adding anything here.
