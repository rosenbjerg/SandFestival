...
Viewport pinning (scrolled-up sessions not getting yanked to the bottom by streaming output) is **SwiftTerm's job as of 1.18.0** — don't reintroduce a `dataReceived` override for it. `AppleTerminalView.updateUserScrollingState` now sets `terminal.userScrolling` from every `scrollTo`/`scroll(toPosition:)`, which is what `Terminal.scroll` has always gated on. That also makes the scrollback-full case exact: the trim branch decrements `yDisp` per trimmed line to hold the text still, where our old override could only clamp and so drifted toward the bottom. Through 1.17 the view layer set only its own same-named property (scrollbar-knob drags, never read), which is why the workaround existed at all. If pinning regresses after a bump, check that wiring before re-adding anything here.
